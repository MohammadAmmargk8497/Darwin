...
def evaluate_search() -> dict:
    ...
    print(f"\n  {'Metric':<28} {'Baseline':>9} {'Structured':>11} {'Delta':>8}")
    print(f"  {'-' * 58}")
    comparisons = [
    ...
    for label, key, fmt, hib in comparisons:
        ...
        delta = s - b
        arrow = ("UP" if delta > 0 else "DN") if hib else ("UP" if delta < 0 else "DN")
        if (delta > 0) == hib:
            wins += 1
...
def evaluate_download() -> dict:
    ...
    if not existing:
        print("  WARN  No papers in papers/ -- skipping download dedup tests")
...
def evaluate_pdf() -> dict:
    ...
    existing = [f.replace(".pdf", "") for f in os.listdir(PAPER_STORAGE) if f.endswith(".pdf")]
    if not existing:
        print("  WARN  No papers in papers/ -- skipping PDF extraction tests")
...
def evaluate_e2e() -> dict:
    ...
    if not search_ok:
        print("  FAIL  Search returned nothing -- cannot continue pipeline test.")
